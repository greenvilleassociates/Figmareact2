...
const defaultData: AssignmentDetailData = {
  ...
  overview: 'This assignment introduces the fundamentals of HTML structure. Students will learn how to create a basic webpage using semantic HTML elements including headers, paragraphs, lists, and links.',
  objectives: [
    'Understand the basic structure of an HTML document',
    'Learn to use semantic HTML5 elements',
    'Create proper document hierarchy with heading tags',
    'Implement lists and links effectively'
  ],
  requirements: [
    'Valid HTML5 document structure',
    'Minimum of 3 sections with proper headings',
    'At least one ordered and one unordered list',
    'Include at least 5 hyperlinks',
...
export function Assignment1() {
  ...
  const loadAssignmentData = (projectid: string) => {
    const saved = localStorage.getItem(`${projectid}a1.json`);
    if (saved) {
  ...
  const saveAssignmentData = (dataToSave: AssignmentDetailData) => {
    if (projectId) {
      ...
      const existingSaved = localStorage.getItem(`${projectId}a1.json`);
      let existingData = existingSaved ? JSON.parse(existingSaved) : {};
      
      // Update with new detail data
      existingData.detailData = dataToSave;
      
      localStorage.setItem(`${projectId}a1.json`, JSON.stringify(existingData));
      setData(dataToSave);
    }
  };
