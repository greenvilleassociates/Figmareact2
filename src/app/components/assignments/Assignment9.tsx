...
const defaultData: AssignmentDetailData = {
  ...
  overview: 'Persistent data storage in the browser. Learn to save, retrieve, and manage data using local storage to create stateful web applications.',
  objectives: [
    'Store data in localStorage',
    'Retrieve and parse stored data',
    'Update and delete stored items',
...
export function Assignment9() {
  ...
  const loadAssignmentData = (projectid: string) => {
    const saved = localStorage.getItem(`${projectid}a9.json`);
    if (saved) {
  ...
  const saveAssignmentData = (dataToSave: AssignmentDetailData) => {
    if (projectId) {
      const existingSaved = localStorage.getItem(`${projectId}a9.json`);
      let existingData = existingSaved ? JSON.parse(existingSaved) : {};
      existingData.detailData = dataToSave;
      localStorage.setItem(`${projectId}a9.json`, JSON.stringify(existingData));
      setData(dataToSave);
    }
  };
