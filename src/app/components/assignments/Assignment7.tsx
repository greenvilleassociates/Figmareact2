...
const defaultData: AssignmentDetailData = {
  ...
  overview: 'Implement client-side form validation using JavaScript. Learn to validate user input, provide feedback, and ensure data quality before submission.',
  objectives: [
    'Validate form fields on submission',
    'Check for required fields and patterns',
...
export function Assignment7() {
  ...
  const loadAssignmentData = (projectid: string) => {
    const saved = localStorage.getItem(`${projectid}a7.json`);
    if (saved) {
  ...
  const saveAssignmentData = (dataToSave: AssignmentDetailData) => {
    if (projectId) {
      const existingSaved = localStorage.getItem(`${projectId}a7.json`);
      let existingData = existingSaved ? JSON.parse(existingSaved) : {};
      existingData.detailData = dataToSave;
      localStorage.setItem(`${projectId}a7.json`, JSON.stringify(existingData));
      setData(dataToSave);
    }
  };
