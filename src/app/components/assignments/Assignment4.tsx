...
const defaultData: AssignmentDetailData = {
  ...
  overview: 'Introduction to JavaScript programming. Learn variables, data types, operators, control structures, and functions to add interactivity to your web pages.',
  objectives: [
  ...
  requirements: [
    ...
    'Use at least one loop (for or while)',
    'Write 3 custom functions with parameters',
    'Display output in console and on webpage'
  ],
...
export function Assignment4() {
  ...
  const loadAssignmentData = (projectid: string) => {
    const saved = localStorage.getItem(`${projectid}a4.json`);
    if (saved) {
  ...
  const saveAssignmentData = (dataToSave: AssignmentDetailData) => {
    if (projectId) {
      const existingSaved = localStorage.getItem(`${projectId}a4.json`);
      let existingData = existingSaved ? JSON.parse(existingSaved) : {};
      existingData.detailData = dataToSave;
      localStorage.setItem(`${projectId}a4.json`, JSON.stringify(existingData));
      setData(dataToSave);
    }
  };
