...
const defaultData: AssignmentDetailData = {
  ...
  overview: 'Learn the fundamentals of CSS styling. This assignment covers selectors, properties, the box model, and basic layout techniques to style your HTML documents.',
  objectives: [
    'Master CSS selectors (element, class, ID)',
    'Understand the CSS box model',
    'Apply colors, fonts, and text styling',
    'Create basic layouts with positioning'
  ],
  requirements: [
    'External CSS stylesheet linked to HTML',
    'Use at least 10 different CSS properties',
    'Style with classes and IDs appropriately',
...
export function Assignment2() {
  ...
  const loadAssignmentData = (projectid: string) => {
    const saved = localStorage.getItem(`${projectid}a2.json`);
    if (saved) {
  ...
  const saveAssignmentData = (dataToSave: AssignmentDetailData) => {
    if (projectId) {
      const existingSaved = localStorage.getItem(`${projectId}a2.json`);
      let existingData = existingSaved ? JSON.parse(existingSaved) : {};
      existingData.detailData = dataToSave;
      localStorage.setItem(`${projectId}a2.json`, JSON.stringify(existingData));
      setData(dataToSave);
    }
  };
